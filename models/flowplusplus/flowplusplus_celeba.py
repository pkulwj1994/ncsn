...
# for celeba flow++
# set use attn = False for dequantizer, set use attn = True for flow
# no conv1x1 (InvConv)
# more blocks 10 --> 16
# dropout = 0.0


class FlowPlusPlus_celeba(nn.Module):
    """Flow++ Model
    Based on the paper:
    ...
    def __init__(self,
                 ...
                 use_attn=True,
                 drop_prob=0.0):
        super(FlowPlusPlus_celeba, self).__init__()
    ...
    def to_logits(self, x, sldj):
        ...
        y = (2 * x - 1) * self.bounds
        #y = x * self.bounds # assume data range is -1, 1
        y = (y + 1) / 2
        y = y.log() - (1. - y).log()
...
class _Dequantization(nn.Module):
    ...
    def forward(self, x, sldj):
        ...
        for i, flow in enumerate(self.flows):
            u, sldj = flow(u, sldj, aux=aux) if i % 3 == 1 else flow(u, sldj)
